...
def clean_missing_values(reviews):
    ...
    if na_values == 0:
        ...
        logger.info(f"After removing missing values, the dataset shape is: {reviews.shape}")
...
def clean_empty_values(reviews):
    ...
    if empty_text == 0 and empty_grade == 0:
        ...
        reviews = reviews[~reviews["reviewText"].str.strip().eq("") & ~(reviews["overall"] == "")].reset_index(drop=True)
        logger.info(f"After removing empty values, the dataset shape is: {reviews.shape}")

    return reviews


def clean_text(reviews):
    reviews["reviewText"] = reviews["reviewText"].str.lower().str.replace(r'[^a-z\s]', '', regex=True)
