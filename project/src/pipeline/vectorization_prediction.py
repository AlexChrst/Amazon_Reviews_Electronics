...
def vectorize_texts(reviews):
    ...
    """
    vectorizer_tf_idf = TfidfVectorizer(ngram_range=(1, 2),  # We tell the algorithm to add to its vocabulary all the single words (unigrams) and pairs of consecutive words (bigrams)
...
def metrics_train(y_train, model, X_train):
    ...
    logger.info(f"Accuracy Score: {accuracy_score(y_train, train_predictions) * 100:.2f}%")